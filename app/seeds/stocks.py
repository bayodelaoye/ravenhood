...
def seed_stocks():
    ...
    for i in stocks:
        ...
        new_stock = Stock(
            ...
            current_price=round(stock.history()['Close'].iloc[-1], 2) if not stock.history().empty else -1,
            ...
            low_today=round(stock.get_info()['dayLow'], 2) if 'dayLow' in stock.get_info() else -1,
            open_price=round(stock.get_info()['open'], 2) if 'open' in stock.get_info() else -1,
            volume=stock.get_info()['volume'] if 'volume' in stock.get_info() else -1,
            fifty_two_week_high=round(stock.get_info()['fiftyTwoWeekHigh'], 2) if 'fiftyTwoWeekHigh' in stock.get_info() else -1,
            fifty_two_week_low=round(stock.get_info()['fiftyTwoWeekLow'], 2) if 'fiftyTwoWeekLow' in stock.get_info() else -1,
            )
